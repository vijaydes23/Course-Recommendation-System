...
# --- Custom CSS (Optimized for Compactness) ---
CUSTOM_CSS = """
<style>
...
/* ------------------ Sidebar Navigation Styling (COMPACT) ------------------ */
...
/* Navbar Buttons in Sidebar - Optimized Padding */
[data-testid="stSidebar"] .stButton>button {
    ...
    padding: 8px 15px; /* Reduced padding for height */
    width: 100%;
    text-align: left;
...
/* Skill Tags (Next Logical Skill Highlight) */
.skill-tag {
...
/* Primary Button Styling (Enroll Now) */
.primary-btn {
    background-color: var(--vibrant-blue); 
    color: white;
    padding: 10px 15px; /* Optimized padding */
    border-radius: 10px;
    width: 100%;
    font-weight: bold;
    margin-top: 8px; 
...
/* Profile Card and Metric Styling (Unchanged) */
.info-box {
    padding: 20px;
    border-radius: 12px;
    background-color: var(--secondary-background-color);
...
class CourseRecommender:
    ...
    def preprocess_data(self):
        ...
        self.df['combined_features'] = (
            ...
            self.df['category_clean'] * 5 + ' ' +
            self.df['skills_clean'] * 3          
        )
...
# --- AI Helper Function: Personalized Justification ---
...
# --- AI Helper Function: Next Logical Skill ---
...
def show_home_page():
    # Title change requested by the user
    st.markdown("<h1 style='text-align: center;'>AI-Powered Course Recommendation System</h1>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col_ai_info:
        ...
        st.markdown("""
        <div class="info-box">
            <p>Our AI uses advanced machine learning to find the perfect courses for you from **400+ options**.</p>
            <ul>
                <li>**Smart Matching:** Uses TF-IDF for accurate course-to-profile matching.</li>
                <li>**Personalized Justification:** Explains *why* each course fits your goals.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_setup_info:
        ...
        <div class="info-box" style="border-left: 5px solid var(--accent-orange);">
            <p>Ready to start your learning journey? It's simple:</p>
            <ol>
                <li>**Setup Profile:** Tell us your skills and interests in **👤 User Profile**.</li>
                <li>**Get Recs:** Go to **💡 Get Recommendations** for tailored course suggestions.</li>
                <li>**Track Progress:** Mark courses **✅ Completed** to keep your learning path updated.</li>
            </ol>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    st.set_page_config(
        page_title="AI-Powered Course Recommendation System",
        page_icon="🎓",
        layout="wide",
    ...
    with st.sidebar:
        ...
        # Deleted: st.markdown("### Intelligent Learning Recommender")
        st.markdown("---")
        ...
        st.markdown("---")
        # Deleted: st.markdown("### ℹ️ System Status")
        st.info(f"""
        **Courses:** {len(st.session_state.recommender.df)}
        **Skills:** {len(ALL_TECHNICAL_SKILLS)}
